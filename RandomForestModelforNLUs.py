...
class Model_VA:
    def __init__(self,mono,splitby):
        ...
        self.Ext_dir = glob.glob("AudioSet_Test_Sliced/"+"/**/*.wav")
...
#maybe use the feature name to define the split instead
    ...
    def opensmileLLD_pipeline_RF_Ext(self, n_factors):
        ...
        Test_data_V = self.opensmile_LLDs_Build_df(self.Ext_dir)
        Train_data_A = Train_data_V
        Test_data_A = Test_data_V
        Train_data_scaled, Test_data_scaled = self.scaled_frames_opensmileLLD(Train_data_V, Test_data_V, self.All_Train_dir, self.Ext_dir)
        ...
        numframes_Test = self.numframes_opensmileLLD(self.Ext_dir)
...
if __name__ == '__main__':
    #run 10 loops on 560 to get predictions for all 560 files:
    main_dir = glob.glob('Mono/*.wav')
    split = 1
    InferModel = Model_VA(main_dir,split)
    n_factors_LLD = 0
    predV, predA = InferModel.opensmileLLD_pipeline_RF_504_56(n_factors_LLD)

    filenames_test_data = []

    for j in range(1,11):
        test_dest = glob.glob("Feb2022/504_56_{0}/Test/*.wav".format(j)) 
        for file in test_dest:
            filenames_test_data.append(file)

    pred_files = []
        
    for i in range(len(filenames_test_data)):
        pred_files.append((filenames_test_data[i],predV[i],predA[i]))
